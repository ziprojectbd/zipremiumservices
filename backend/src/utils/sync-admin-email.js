...
export async function syncAdminEmail() {
    ...
    try {
        ...
        const result = await User.updateMany({ email: { $ne: ADMIN_EMAIL }, role: 'admin' }, { role: 'customer' });
        ...
        devLog(`📊 Current admin users: ${adminUsers.map((u) => u.email).join(', ')}`);
    }
    catch (error) {
        devLog('❌ Error syncing admin email:', error.message);
    }
}
//# sourceMappingURL=sync-admin-email.js.map