...
- Return ONLY valid JSON. No explanations, no markdown formatting.`;
    ...
    for (const model of models) {
      try {
        ...
        const features = Array.isArray(data.features)
          ? data.features.slice(0, 5).map((f) => String(f).trim())
          : [];
    ...
    const message = lastError?.message?.includes('429') || lastError?.message?.includes('quota') || lastError?.message?.includes('RESOURCE_EXHAUSTED')
